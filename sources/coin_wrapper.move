module yield::coin_wrapper {
    ...
    public entry fun initialize() {
        ...
        let yield_signer = &yield_package_manager::get_signer();
        let (coin_wrapper_signer, signer_cap) = account::create_resource_account(yield_signer, COIN_WRAPPER_NAME);
        yield_package_manager::add_address(string::utf8(COIN_WRAPPER_NAME), signer::address_of(&coin_wrapper_signer));
    ...
    public fun is_initialized(): bool {
        yield_package_manager::address_exists(string::utf8(COIN_WRAPPER_NAME))
    }

    #[view]
    /// Return the address of the resource account that stores all deposited coins.
    public fun wrapper_address(): address {
        yield_package_manager::get_address(string::utf8(COIN_WRAPPER_NAME))
    }
